...
mod benches {
    ...
    #[bench]
    fn dscal(bencher: &mut Bencher) {
        let mut rng = rand::thread_rng();
        let vals: Vec<f64> = (0..SIZE).map(|_|rng.gen()).collect();
        let x = arr1(&vals);

        bencher.iter(|| x.mapv(|v|v*2.));
    }
    #[bench]
    fn daxpy(bencher: &mut Bencher) {
        let mut rng = rand::thread_rng();
        let x_vals:Vec<f64> = (0..SIZE).map(|_|rng.gen()).collect();
        let y_vals:Vec<f64> = (0..SIZE).map(|_|rng.gen()).collect();
    ...
    #[bench]
    fn dgemv(bencher: &mut Bencher) {
        let mut rng = rand::thread_rng();
        let sqrt_size = (SIZE as f32).sqrt() as usize;
        let a_vals:Vec<f64> = (0..sqrt_size * sqrt_size).map(|_|rng.gen()).collect();
        let x_vals:Vec<f64> = (0..sqrt_size).map(|_|rng.gen()).collect();
        let y_vals:Vec<f64> = (0..sqrt_size).map(|_|rng.gen()).collect();
    ...
    #[bench]
    fn dgemm(bencher: &mut Bencher) {
        let mut rng = rand::thread_rng();
        let sqrt_size = (SIZE as f32).sqrt() as usize;
        let a_vals:Vec<f64> = (0..sqrt_size*sqrt_size).map(|_|rng.gen()).collect();
        let b_vals:Vec<f64> = (0..sqrt_size*sqrt_size).map(|_|rng.gen()).collect();
        let c_vals:Vec<f64> = (0..sqrt_size*sqrt_size).map(|_|rng.gen()).collect();
